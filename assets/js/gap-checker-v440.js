...
async function loadSongs() {
  ...
  try {
    const response = await apiGet("gap", {});
    ...
    if (
      initialSongId &&
      songs.some(item => item.songId === initialSongId)
    ) {
      el.songSelect.value = initialSongId;
      el.status.textContent = "曲を選択済みです。基準日を選択してください。";
      syncButton();
    } else {
      el.status.textContent = "曲と基準日を選択してください。";
    }
  } catch (error) {
    console.error(error);
