...
function setError(error){e.status.hidden=false;e.status.classList.add("error");e.status.innerHTML=`<strong>ホームデータを取得できませんでした。</strong><span>${escapeHtml(error?.message||"不明なエラー")}</span>`;e.retryButton.hidden=false}
...
function renderHome(d){recentItems=d.recentPerformances||[];recentVisibleCount=5;renderSummary(d.summary||{});renderFeatured(d.featured||{});renderToday(d.today||{});renderTopSongs(d.topSongs||[]);renderTopVenues(d.topVenues||[]);renderLatestEvents(d.latestEvents||[]);renderRecent();e.status.hidden=true;e.summary.hidden=false;e.featuredSection.hidden=false;e.topSection.hidden=false;e.middleSection.hidden=false;e.latestEventsSection.hidden=false;e.recentSection.hidden=false}
