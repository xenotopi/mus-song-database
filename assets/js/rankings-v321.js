...
function updateSortOptions() {
  ...
  if (currentFromUrl && options.some(([value]) => value === currentFromUrl)) {
    el.rankingSort.value = currentFromUrl;
  }
}
...
function sortItems(items) {
  ...
  return items.sort((a, b) => {
    ...
    if (sort === "singer-desc") {
      return Number(b.totalSingerCount || 0) - Number(a.totalSingerCount || 0);
    }
...
function getItems() {
  const source = Array.isArray(data[activeTab]) ? data[activeTab].slice() : [];
  const query = normalize(el.rankingSearch.value);

  const filtered = query
    ? source.filter(item => normalize(getSearchText(item)).includes(query))
    : source;
  ...
  el.eventsList.innerHTML = items.length
    ? items.map((item, index) => rankRow({
        ...
        meta: [
          ...
          `<span>延べ歌唱人数 ${Number(item.totalSingerCount || 0)}人</span>`,
          `<span>平均 ${Number(item.averageSingerCount || 0)}人</span>`
        ].join(""),
        count: `${Number(item.uniqueSongCount || 0)}曲`,
        countLabel: "重複なし"
      })).join("")
...
function renderVenues(items) {
  el.venuesList.innerHTML = items.length
    ? items.map((item, index) => rankRow({
        ...
        count: `${Number(item.eventCount || 0)}件`,
        countLabel: "開催イベント"
      })).join("")
...
function renderSingers(items) {
  el.singersList.innerHTML = items.length
    ? items.map((item, index) => rankRow({
        ...
        count: `${Number(item.performanceCount || 0)}件`,
        countLabel: "歌唱記録"
      })).join("")
