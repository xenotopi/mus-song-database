...
function renderSongRecords_() {
  ...
  const completeCount =
    Number(
      currentSong?.statistics?.completePerformanceCount ??
      currentSong?.statistics?.completeSongCount ??
      0
    );

  const completeRate =
    performances.length
      ? Math.round(completeCount / performances.length * 100)
      : 0;
...
elements.yearTimeline.addEventListener(
  ...
  event => {
    ...
    if (selectedYear !== "all") {
      elements.historySection.scrollIntoView({
        behavior: "smooth",
        block: "start"
      });
    }
