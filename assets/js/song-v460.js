...
const elements = {
  ...
  songMemberLine: document.getElementById("songMemberLine"),
  ...
  analysisGapCheckerLink: document.getElementById("analysisGapCheckerLink"),
...
const MEMBER_COLORS_ = [
  {
    color: "#f39a3d",
    keys: ["高坂穂乃果", "穂乃果", "新田恵海", "新田"]
  },
  {
    color: "#5bc0de",
    keys: ["絢瀬絵里", "絵里", "南條愛乃", "南條"]
  },
  {
    color: "#b8b8c8",
    keys: ["南ことり", "ことり", "内田彩", "内田"]
  },
  {
    color: "#3b74c5",
    keys: ["園田海未", "海未", "三森すずこ", "三森"]
  },
  {
    color: "#f2c94c",
    keys: ["星空凛", "凛", "飯田里穂", "飯田"]
  ...
  {
    color: "#52c46a",
    keys: ["小泉花陽", "花陽", "久保ユリカ", "久保"]
  },
  {
    color: "#ef71b8",
    keys: ["矢澤にこ", "にこ", "徳井青空", "徳井"]
  }
];

const ALL_MEMBER_COLORS_ =
  MEMBER_COLORS_.map(item => item.color);


function normalizeMemberSource_(value) {
  if (Array.isArray(value)) {
    return value
      .flatMap(normalizeMemberSource_)
      .filter(Boolean);
  }
...
function getRegularSingerNames_(song) {
  const candidates = [
    ...
    song?.regularSingerNames,
    song?.originalSingerNames,
    song?.regularSinger,
    song?.originalSinger,
    song?.["正規歌唱者"],
    song?.["正規歌唱者名"]
...
function resolveMemberColors_(song) {
  const names =
    getRegularSingerNames_(song);

  if (!names.length) {
    return ALL_MEMBER_COLORS_;
  }
  ...
  names.forEach(name => {
    ...
    MEMBER_COLORS_.forEach(member => {
      if (
        member.keys.some(key =>
          normalized.includes(
            String(key)
              .normalize("NFKC")
              .replace(/\s+/g, "")
...
function renderSongMemberLine_(song) {
  if (!elements.songMemberLine) {
    return;
  }

  const colors =
    resolveMemberColors_(song);

  elements.songMemberLine.style
    .setProperty(
      "--member-count",
      String(colors.length)
    );

  elements.songMemberLine.innerHTML =
    colors
      .map(color =>
...
function renderHistory_() {
  ...
  if (!elements.historyMoreButton.hidden) {
    elements.historyMoreButton.innerHTML =
      `<strong>もっと見る</strong><small>残り${filtered.length - visible.length}件</small>`;
  }
}
...
function buildSongDiscovery_(song) {
  ...
  const discoveryCards = [
    ...
    {
      label: "CHECK THE GAP",
      title: "この曲の「いつ振り」を調べる",
      meta: "曲を選択した状態でチェッカーを開きます",
      href:
        `gap-checker.html?song=${encodeURIComponent(
          songKey
        )}`,
      checker: true
    }
  ...
  elements.songDiscovery.innerHTML =
    discoveryCards.map((card, index) => `
      <a
        class="discovery-card ${index < 2 ? "discovery-card-performance" : ""} ${card.checker ? "discovery-card-checker" : ""}"
        href="${card.href}"
      >
    ...
    metrics.yearly.length
      ? `
          ...
          ${metrics.yearly.map(item => {
            ...
            return `
              <div class="song-year-row ${metrics.peakYear && item.year === metrics.peakYear.year ? "peak" : ""}">
...
function renderSong(song) {
  ...
  renderSongMemberLine_(song);

  if (elements.analysisGapCheckerLink) {
    elements.analysisGapCheckerLink.href =
      `gap-checker.html?song=${encodeURIComponent(
        song.songId || songId
      )}`;
  }
